...
var downbtn2 = document.getElementById('down2').value;
var upbtn2 = document.getElementById('up2').value;
  ...
  for(var i=0;i<elements.length;i++)
{
	   ...
	   if (moreBtn.className == thisname)
	   {
		 ...
		 { 
			...
			document.getElementById('moreWinImg'+windowNo).src = upbtn2;
		 }
		else 
		{
			elements[i].style.display = 'none';
			//document.getElementById('moreWin'+windowNo).innerHTML = 'View more manufacturers';
			document.getElementById('moreWinImg'+windowNo).src = downbtn2;
		}
	  }
...
function signIn()
{
	var signInPathToFOS=$('#signInPath').val();

	var unm=$('#username_cust').val();
	var uname=JSON.stringify(unm);
	var passwd=$('#password_cust').val();
	var pass=JSON.stringify(passwd);
	
	$.post(signInPathToFOS,{
				username:uname, 
				password:pass, 
				},function(data)
				{ 
					if(data.responseCode==200 )
					{  
						if(data.messaage=="signin" ) {
							//alert("logged in");
							window.location = data.url;
						}
						else if(data.messaage=="error" ){
							//alert("invalid user name or password");
							$("#invalid_login").html("<p style=\"color:#D63333\">Invalid email or password </p>");
						}
